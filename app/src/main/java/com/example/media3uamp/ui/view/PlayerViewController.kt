...
class PlayerViewController @JvmOverloads constructor(
    ...
    private fun initView(context: Context) {
        ...
        seekBar.progressDrawable = ContextCompat.getDrawable(this.context, R.drawable.seekbar_style_drawables)
    ...
    companion object {
        ...
        fun applyScale(view: View, scale: Float, duration: Int) {
            ...
            view.setOnTouchListener { v, event ->
                when (event.action) {
                    MotionEvent.ACTION_DOWN -> v.animate().scaleX(scale).scaleY(scale).setDuration(duration.toLong()).start()
